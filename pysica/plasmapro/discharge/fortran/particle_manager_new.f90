...
contains
   ...
   subroutine add_particles(x, y, z, vx, vy, vz, v, v2, &
      ...
      integer,                                       intent(in)    :: n_cells              ! Number of PIC cells
...
!            call pause
   ...
   contains
      ...
      subroutine rescale_particles(type, factor, n_active)
 
          ! Rescale the number of particles of the specified type by a factor f

          ! Parameters
          integer,  intent(in)  :: type, n_active                           ! type of particle and number of active particles
          real(dp), intent(in)  :: factor                                   ! Rescale factor

          ! Local variables
          real(dp)              :: delta                                    ! Length of each PIC cell
          real(dp), allocatable :: z_limit(:)                               ! Cell boundaries
          integer               :: i_pic, i_particle, i_cell
          integer               :: cell_n_particles(:)                      ! Number of particles in each cell
          integer, allocatable  :: cell_particle_indexes(:,:)               ! Indexes of particles in each cell

          delta = distance / real(N_cells)

          allocate(z_limit(0:n_cells))          
          allocate(cell_n_particles(1:n_particles))
          allocate(cell_particle_indexes(1:n_particles, 1:n_cells))

          ! Calculate cell boundaries and erase number of particles in each cell
          z_limit(0) = 0.0
          do i_cell = 1, N_cells
             z_limit(i_cell) = z_limit(i_cell-1) + delta
             cell_n_particles(i_cell) = 0             
          enddo

          ! Count number of particles in each cell and save their indexes
          do i_particle = 1, n_particles
             ! Check in which cell this particle is located
             do i_cell = 1, n_cells
                ! If the particle is in this cell, save its index and exit the inner loop (since it can be in one cell only)
                if (       isactive(type, i_particle) &
                    &.and. (z(type, i_particle) >= z_limit(i_cell-1)) &
                    &.and. (z(type, i_particle) <  z_limit(i_cell)) ) then
                   ! Increase the number of particles in this cell
                   cell_n_particles(i_cell) = cell_n_particles(i_cell) + 1
                   ! Save the index of this cell in the array column of this particle
                   cell_particle_indexes(cell_n_particles(i_cell), i_cell) = i_particle
                   exit
                endif
             enddo ! i_cell = 1, N_cells
          enddo ! i_particle = 1, n_particles

          ! In each cell, rescale the number of particles, if it is not empty
          do i_cell = 1, n_cells
             if (cell_n_particles(i_cell) > factor) rescale_cell(i_cell)
          enddo

       contains

          subroutine rescale_cell(i_cell)

             ! Parameters
             integer, intent(in) :: i_cell

             ! Local variables
             integer              :: i_particle, i_bin, n_particles_bin
             integer              :: n_bins              ! Number of bins in the speed distribution of this cell
             logical, allocatable :: h_mask(:,:)
             integer              :: n_remove_bin
             integer, allocatable :: tofalse_indexes(:)
             
             logical
             real(dp)             :: delta_v             ! Dimension of the bins             
             real(dp)             :: v_min, v_max        ! Maximum and minimum speed of the particles in this cell
             real(dp)             :: v

             if ( cell_n_particles(i_cell)) > 4 ) then
                n_bins = int(sqrt(real(cell_n_particles(i_cell))))
             else
                nbins = 1
             endif

             allocate(h_mask(n_particles, n_bins))

             call generate_histogram_mask(h_mask, v_min, v_max, delta_v, &
                                         &(type, cell_particle_indexes(:, i_cell)), n_bins, cell_n_particles(i_cell))

             ! Cycle over all bind of the speed histogram of this cell
             do i_bin = 1, n_bins
                ! Get the number of particles in this bin of the speed histogram
                n_particles_bin = count(h_mask(:, i_bin))
                if (n_particles_bin >= int(factor)) then
                   ! Calculate the number of particles to remove in this bin
                   n_remove_bin = real(n_particles_bin) / factor

                endif
             enddo

          end subroutine rescale_cell
          ...
          deallocate(cell_n_particles, cell_particle_indexes)
